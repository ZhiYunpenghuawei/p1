...
"""Run one isolated public beam_search_v1 case without benchmark instrumentation."""
...
def main() -> None:
    ...
    table_path = args.output.with_name("a5_constraint_table.safetensors")
    artifact_digest, tokenizer_digest, _ = build_constraint_table(
        triples_path=args.catalog.resolve(),
    ...
    with GRLLM(
        ...
        def run() -> dict[str, Any]:
            ...
            output = llm.beam_search_v1([{"prompt_token_ids": prompt[:-1]}], params)
    ...
    write_json(
        args.output,
        {
            ...
            "boundary": (
                "public beam_search_v1 through lazy output creation; full Beam field "
                "materialization is measured separately"
            ),
            "config": {
                "width": args.width,
