...
| 层级 | 二进制 | 十进制 |
|------|--------|--------|
| `sa` | `0000 0000 0000 01` | 1 |
| `sb` | `0000 0000 0000 10` | 2 |
| `sc` | `0000 0000 0000 11` | 3 |

---

#### 2. 前缀序列构造

每个 SID 在输入序列中占 4 个 token：
[ctx, sa, sb + 8192, sc + 16384]

text

代入 `(sa=1, sb=2, sc=3)`：
[0, 1, 2 + 8192, 3 + 16384] = [0, 1, 8194, 16387]
...
设 `bf0=2, bf1=2, bf2=2, beam=2`：

**Step 0 — 预测 sa**：
输入: [历史序列, 0] → head_0 → top-2: [1, 5]
...
候选 4 条：`(1,2), (1,8), (5,3), (5,9)`

**Step 2 — 预测 sc**：

拼接时 **sb 要加 8192**：
(1,2) → [..., 0, 1, 8194] → head_2 → [3, 7]
(1,8) → [..., 0, 1, 8200] → head_2 → [4, 10]
(5,3) → [..., 0, 5, 8195] → head_2 → [6, 12]
(5,9) → [..., 0, 5, 8201] → head_2 → [8, 15]

text

共 8 条完整候选，按总分排序，保留 top‑2：
候选1: (1, 2, 3) → 拼回原 SID: 0000 0000 0000 11 0000 0000 0000 10 0000 0000 0000 01
候选2: (1, 2, 7) → 拼回原 SID: 0000 0000 0000 11 0000 0000 0000 10 0000 0000 0000 11
...
| 要点 | 说明 |
|------|------|
| **切分** | 39 位 SID → 3 个 13 位值：`sa`（低位）、`sb`（中位）、`sc`（高位） |
| **偏置** | 构造输入时：`sa+0`，`sb+8192`，`sc+16384`，映射到词表三个区间 |
| **推理** | 三步自回归：`sa` → `sb` → `sc`，每步依赖上一步结果 |
| **head 输出** | 每个 head 输出维度 8192，对应 13 位值的 8192 种可能 |
| **输出** | 取得分最高的 `beam` 个完整三元组，拼回 39 位 SID |
