...
- `config.json` 缺少权重转换所需的架构字段。
- 检查点不是代码预期的 Megatron 键名或不是单 rank、`EP=1` 布局。
- 嵌入权重、专家数量、隐藏维度或预测头形状与配置不一致。
- `external_rank0.pt` 不包含 `lm_heads` 或三个 `heads.N.weight`。
- Transformers 版本没有 Qwen3-MoE 支持，或其内部参数命名与当前映射不兼容。
- 历史 SID 不遵循当前 14 位步长的打包约定。
- 历史过长或分支规模过大，导致设备内存不足。

`torch.load(..., weights_only=False)` 会反序列化完整 PyTorch 文件，因此只应加载可信来源的
检查点。
...
- 验证单 rank RECIF 检查点能否正确转换并在 Hugging Face Qwen3-MoE 上运行。
- 对少量用户历史执行离线 Top-K 下一物品 SID 预测。
- 对比不同 `bf` 和 `beam` 配置下的候选结果。
- 作为后续实现批处理、KV Cache、目录约束或服务化推理时的参考基线。

它目前不直接提供在线服务、跨请求动态批处理、多 rank 权重合并、目录约束或面向大规模吞吐
优化的完整推理后端。
