...
每个物品的语义 ID（Semantic ID）由三个层级组成，分别对应低、中、高位：

| 层级 | 名称 | 位宽 | 取值范围 | 偏移量 |
|------|------|------|----------|--------|
| 低位 | `sa` | 13 bits | [0, 8191] | `+ 0 × 8192` |
| 中位 | `sb` | 13 bits | [0, 8191] | `+ 1 × 8192` |
| 高位 | `sc` | 13 bits | [0, 8191] | `+ 2 × 8192` |

每个 SID 三元组 `(sa, sb, sc)` 在模型内部被表示为三个独立的 token，词表大小为 `3 × 8192 = 24576`。

---

#### 2. 前缀序列构造

每个物品在输入序列中占据 4 个 token 位置，格式如下：
[ctx, sa, sb + 8192, sc + 16384]

其中：
- `ctx`：占位 token，固定为 0
- `sa`：直接使用模型预测的低位值（范围 0–8191）
- `sb + 8192`：中位值加上偏移量 `8192`，映射到词表的中段
- `sc + 16384`：高位值加上偏移量 `16384`，映射到词表的高段

**注意**：尽管 `sa`、`sb`、`sc` 各自由 13 位表示（取值范围 0–8191），但在拼接成前缀序列时，每个值都会被扩展为其对应的词表索引（即加上层级偏移量），以便模型能够正确区分当前预测的是哪个层级的 SID。

---

#### 3. Beam Search 推理流程

推理过程分为三个阶段，逐步生成 `sa`、`sb`、`sc`：

| 步骤 | 输入 | 预测头 | 输出 | 候选数 |
|------|------|--------|------|--------|
| **Step 0** | 用户历史序列末尾的 `ctx` 位置 | `head_0` | 取 top‑`bf0` 个 `sa` 候选 | `bf0` 条 |
| **Step 1** | 每个 `sa` 候选拼接后，在 `sa` 位置 | `head_1` | 每个 `sa` 取 top‑`bf1` 个 `sb` 候选 | `bf0 × bf1` 条 |
| **Step 2** | 每个 `(sa, sb)` 拼接后，在 `sb` 位置 | `head_2` | 每个 `(sa, sb)` 取 top‑`bf2` 个 `sc` 候选 | `bf0 × bf1 × bf2` 条 → 排序后保留 top‑`beam` |

每个候选的得分为三步 log‑softmax 输出之和，代表该三元组的对数概率。

---

#### 4. 关键要点

- **层级偏移**：模型预测的 `sa`、`sb`、`sc` 均为 13 位值（0–8191），但在构造前缀序列时，需要将 `sb` 和 `sc` 分别加上偏移量 `8192` 和 `16384`，以映射到词表的对应区间。
- **三步自回归**：每一层级的预测都依赖于前一层级的预测结果，形成链式依赖。
- **候选扩展与截断**：前两步只做扩展（branching），最后一步统一按得分排序并截断，保留得分最高的 `beam` 个完整三元组。
- **最终输出**：`beam` 个 `(sa, sb, sc)` 三元组及其对应的对数概率分数，用于后续的候选物品检索。
