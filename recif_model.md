...
假设一个 SID 使用 39 bit 表示，每一层占用 13 bit：
...
每个层级本身都是一个独立的 8192 类分类空间。

3. 为什么需要层次偏置（Hierarchical Bias）
...
因此 RecIF 在构建输入序列时，引入 层次偏置（hierarchical bias）。
