...
### 5.2 SchedulerBatchInfo（执行信息：调度器输出给 worker）
...
### 6.3 覆盖 vs 新增 vs 继承（原版 Scheduler）
...
- KV 池按 batch 档 × 长度档预留稳定切片，warmup 后地址固定；
