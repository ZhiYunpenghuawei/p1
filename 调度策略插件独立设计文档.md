...
- **batch 档位**：推荐两级桶（4/8），发车与降级规则见 5.10。
...
- `S` = decode 步数（RECIF = 3）；
- `W` = beam 宽档位数（如 4/8 → 2）；
- `B` = decode batch 档位数（**推荐两级 4/8 → 2**，见 5.10；最多 3 级）；
...
### 5.10 batch 大小策略：两级桶（4/8）与降级发车（含图要求）
...
scheduler_plugin:
  ...
  buckets: [4, 8]                     # decode batch bucket：8 常态、4 降级（见 5.10）
  normal_bucket: 8                    # 常态满批发车档位
  degraded_bucket: 4                  # 降级发车档位（超时 / deadline / 高优）
...
| 两级 batch（4/8，5.10） | batch bucket 1/2/4/8/16 + padding | batch 取 2 的幂档 |
| 图数量估算（5.9） | 图 key 各维度乘积 | `bs × seq` 组合数 |
