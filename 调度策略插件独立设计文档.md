...
请求到达
  │
  ▼
① 按 group_hint 分车（业务分组相同者优先同一辆；没有 hint 就跳过）
  │
  ▼
② 按预估长度归入长度 bucket（900/1000 → 1024）
  │
  ▼
③ 进入该 bucket 的等待队列（队列内按 priority 排序）
  │
  ▼
④ 引擎空闲或 on_tick 触发“组批检查”
  │
  ├─ 4a. 队列里有请求吗？
  │     ├─ 没有 → 什么都不发，等下一个请求
  │     └─ 有 ↓
  ├─ 4b. 这个 bucket 已经有图了吗？（available_graph_buckets）
  │     ├─ 没有 → 调 warmup_bucket 请求预捕获，这批先等着
  │     └─ 有 ↓
  ├─ 4c. 满足发车条件了吗？
  │     ├─ 满批（≥ min_batch）
  │     ├─ 或 有高优请求（priority 高，可不满批）
  │     ├─ 或 接近 deadline / 超过 max_wait_ms（强制发车）
  │     ├─ 都不满足 → 继续等，下次再查
  │     └─ 满足 ↓
  └─ 4d. 取队列头部 ≤ max_batch 个请求
           → 生成 BatchSpec { batch_id, request_ids, bucket, deadline, graph_keys }
           → submit_batch 交给引擎
```
...
```text
新请求 req-001 到达（长度 1000，beam 4）
  │
  ▼
┌────────────────────────────────────────────┐
│ 1. 插件 on_request_arrive(req)             │
│    预估长度 1000 → 长度 bucket = 1024       │
│    进入 bucket 1024 的等待队列              │
└────────────────────────────────────────────┘
  │  等待：攒批或超时
  ▼
┌────────────────────────────────────────────┐
│ 2. 插件组批：生成 BatchSpec                 │
│    batch_id=b-xxx, request_ids=[req-001,…] │
│    bucket=1024, graph_keys=[prefill:1024,  │
│    decode:step1:4, step2:4, step3:4]       │
└────────────────────────────────────────────┘
  │
  ▼
┌────────────────────────────────────────────┐
│ 3. 引擎 submit_batch(spec)                 │
│    校验 bucket 1024 是否有图                │
│    预分配 KV 池稳定切片                     │
└────────────────────────────────────────────┘
  │
  ▼
┌────────────────────────────────────────────┐
│ 4. 组执行内核（引擎/worker 内，不再回外层）  │
│    prefill 图（长度 1024）                  │
│    → step1 图（beam select 在步间完成）     │
│    → step2 图                              │
│    → step3 图 → 出最终结果                  │
└────────────────────────────────────────────┘
  │
...
└────────────────────────────────────────────┘
  ...
  ▼
┌────────────────────────────────────────────┐
│ 6. 插件 on_batch_finish(spec, result)      │
│    标记请求完成 → 进入统计                  │
│    可能触发下一批                           │
└────────────────────────────────────────────┘
```
...
```text
┌───────────────────────────────┐
│ SchedulingPlugin（基类/接口）  │
│  on_request_arrive(req)       │
│  on_batch_finish(spec, res)   │
│  on_tick(now, capacity)       │
│  next_batch(now, cap)->spec   │
│  on_capacity_change(cap)      │
└───────────────┬───────────────┘
                │ 持有（组合）
                ▼
┌───────────────────────────────┐        ┌───────────────────────────────┐
│ Policy（策略链）               │        │ EngineBatchInterface（引擎侧）│
│  FCFSPolicy                   │───────►│  submit_batch(spec)->batch_id │
│  LengthClusterPolicy          │ 调用    │  poll_batch(id)->result      │
│  PriorityPolicy               │        │  available_capacity()->cap   │
│  DeadlinePolicy               │        │  warmup_bucket(bucket)       │
│  GroupHintPolicy              │        └───────────────┬───────────────┘
│  GraphAwarePolicy             │                        │ 实现
│  （Combiner 决定最终发射）      │                        ▼
└───────────────────────────────┘        ┌───────────────────────────────┐
                                         │ BatchCoordinator（引擎内）     │
                                         │  准入→预分配→执行→回收         │
                                         └───────┬───────────────┬───────┘
                                                 ▼               ▼
                                    ┌─────────────────┐  ┌─────────────────┐
                                    │GroupExecution   │  │ KVAllocator     │
                                    │Kernel           │  │ 按 bucket 预留  │
                                    │prefill+step1..N │  │ 稳定池切片      │
                                    └────────┬────────┘  └─────────────────┘
                                             ▼
                                    ┌─────────────────┐
                                    │ GraphRegistry   │
...
数据类（在插件与引擎之间传递）：
  PluginRequest ──► BatchSpec ──► BatchResult
                              Capacity（引擎 → 插件，反向）
```
