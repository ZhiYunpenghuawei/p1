...
## 0. 先看别人怎么做：NVIDIA 与 vllm-ACS 的策略总结

> 本节用大白话总结两家怎么做调度与成图；详细源码分析见主文档《调度策略调研与设计》第 1 节，源码位置见第 14 节。

### 0.1 NVIDIA（recsys-examples）：即来即接 + 引擎内按阶段分组 + 图桶

**大白话**：

- 请求来了就接入，不等攒批（连续接入）；
- 执行时引擎把“同一个 decode 阶段、同样 beam 数、同样上下文长度”的请求凑成一个 microbatch，**不同阶段的请求绝不混跑**；
- 成图：每个（阶段 × 形状）第一次见到才录图（懒捕获 + LRU 缓存）；batch 不够档位就补空位到预设档位（1/2/4/8/16）再放录像，跑完切回实际大小；KV 必须住在固定池切片里，地址对不上就重录或退回普通执行（eager）。

**一句话**：NVIDIA = **引擎帮你组批（按阶段），图按固定档位懒捕获，允许回退**。

### 0.2 vllm-ACS：连续调度 + 调用方组 step + 启动全形状预捕获

**大白话**：

- 引擎还是 vLLM 原版连续调度（即来即接），调度器没怎么改；
- beam 搜索路径由**调用方**在每个 decode 步把所有 beam 合并成一次 `generate` 调用——**组 step 由调用方做**；
- KV 按用户（uid）缓存：历史 KV 命中、用户级 LRU 淘汰、Mooncake 搬运；
- 成图：HSTU 模型启动后把（batch 取 2 的幂 × 长度挡位）所有形状的 ACL 图**一次性全部录好**；运行时按实际形状找最近的已录档位；找不到直接报错（无回退）。

**一句话**：vllm-ACS = **引擎不组批，调用方组 step，图启动全预捕获，无回退**。

### 0.3 共同点与不同点（简表）

| 维度 | NVIDIA | vllm-ACS |
|---|---|---|
| 谁组批 | 引擎内按 (step, beam, 长度) 分组 | 调用方按 step 把全部 beam 一次提交 |
| 接入方式 | 连续接入 | 连续接入（vLLM 原版） |
| 捕获时机 | 首次见到形状才捕获（懒） | 启动全形状预捕获 |
| 缺图时 | 回退 eager | 报错 |
| 分桶 | batch 1/2/4/8/16 + 长度档 | batch 2 的幂 × 长度挡位 |
| KV | 稳定池切片，地址必须一致 | 固定 slot + 用户级 KV（LRU / Mooncake） |

**共同点**：用图都必须满足“**同形状、同步进、KV 地址稳定**”；都用 bucket 控制形状数量；都区分 prefill 图与 decode 图。

### 0.4 对我们的启示（由此引出我们的策略）

1. **组批放哪一层是可选的设计点**：NVIDIA 放在引擎、ACS 放在调用方——我们想把它做成**可插拔的一层**：用户随便用什么调度策略，最终都翻译成统一的“组 batch 计划”（BatchSpec），引擎只按批跑；
2. **捕获时机取中间**：ACS 全预捕获太刚性，NVIDIA 懒捕获有首见抖动——**常用形状（8×L×W）请求到达即捕获，降级档（4）空闲预热，缺图 eager 兜底**；
3. **batch 档位两级就够**：4/8（8 常态、4 降级），图数量可控。

所以我们的策略是：**调度插件（策略翻译层）+ 组 batch 执行内核 + 两级桶图**。第 1 节开始详细展开。

### 0.5 我们的概念 ↔ 别人实现的对应关系

| 我们的概念 | NVIDIA 对应 | vllm-ACS 对应 |
|---|---|---|
| `available_graph_buckets` | 已捕获的 batch bucket（懒捕获缓存） | 启动预捕获的 `bs_list × seqlen_list` 组合 |
| `warmup_bucket()` / 到达即捕获（5.11） | 首次见到形状才捕获（lazy） | 启动全形状预捕获（无回退） |
| eager 回退 | 支持 | 不支持（缺形状报错） |
...
**NVIDIA：图数量不固定，按“见过的形状”懒捕获，用 LRU 上限控制**。

| 项 | 值 | 源码位置 |
|---|---|---|
| decode 图缓存上限 | 默认 **128 张**（`GR_INFERENCE_DECODE_CUDA_GRAPH_MAX_ENTRIES` 可调） | [decode_cuda_graph.py](/tmp/nvidia_decode_cuda_graph.py:50) |
| prefill 图缓存上限 | 默认 **32 张**（`GR_INFERENCE_PREFILL_CUDA_GRAPH_MAX_ENTRIES` 可调） | [prefill_cuda_graph.py](/tmp/nvidia_prefill_cuda_graph.py:51) |
| decode batch bucket | 默认 **(1, 2, 4, 8)**（`GR_INFERENCE_DECODE_CUDA_GRAPH_BATCH_BUCKETS` 可调）；实际 batch 向上取第一个 ≥ 的档 | [默认值](/tmp/nvidia_continuous.py:889)、[档位选择](/tmp/nvidia_continuous.py:2947) |
| 缓存结构 | `OrderedDict` LRU：命中后 `move_to_end` 提升热度；超过上限淘汰最久未用；`freeze_captures()` 后不再新捕获，未命中走 eager | [LRU 命中](/tmp/nvidia_decode_cuda_graph.py:192)、[存储/淘汰](/tmp/nvidia_decode_cuda_graph.py:227) |
| 一张图对应什么 | decode：一个（step × 形状 × KV 地址）组合一张整图；prefill：piecewise 时一个形状 = embed 1 张 + 层块 N 张 + output 1 张 | [decode 存储](/tmp/nvidia_decode_cuda_graph.py:227)、[prefill 分段](/tmp/nvidia_prefill_cuda_graph.py:182) |
| 图数量公式 | ≤ 见过的（device×dtype×shape×ctx_len×KV view×step×beam×decode_nums）组合数，decode 上限 128、prefill 上限 32 | [decode key](/tmp/nvidia_decode_cuda_graph.py:401)、[prefill key](/tmp/nvidia_prefill_cuda_graph.py:399) |

**vllm-ACS：图数量启动时一次性确定，无缓存淘汰，全部预捕获**。

| 项 | 值 | 源码位置 |
|---|---|---|
| batch 档 | `bs_list = [2**i]` 直到 `max_batch_size`（1/2/4/…） | [hstu.py](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:1050) |
| 长度档 | `seqlen_list = captured_tokens_list`：从 2^8 起按 `graph_step` 递增到 `max_seq`，最后补 max_seq | [ag_demo.py](/tmp/vllm-ACS/model_zoo/gr-ag/ag_demo.py:118) |
| 组合数 | `#bs × #seq`（并满足 seq ≤ max_seq_len、bs×seq ≤ max_num_tokens） | [预捕获循环](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:1071) |
| 每个组合的图数 | embedding 2（prefill/decode）+ dense 1 + pd 一体 1 + prefill（逐层 N 或整网 1）+ decode（N+1 张） | [_capture_model](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:864) |
| 缓存结构 | 字典 `[bs][seq]` 直查：`_prefill/_decode/_pd/_embedding/_dense/_attn_metadatas`；**无 LRU、无淘汰、运行时无新捕获**，缺形状直接报错 | [forward_aclgraph 查询](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:577) |
| 示例（默认 demo 参数） | max_batch=1、max_seq=10240、graph_step=500 → **22 个长度档 × 1 个 batch 档 = 22 组合**；N=4 层、非逐层 prefill 时每组合 10 张 → **约 220 张图**（逐层 prefill 则每组合 13 张 → 286 张） | [capture_model](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:1038) |

---

## 1. 我们的策略：调度插件 + 组 batch（为什么需要）
...
flowchart TD
    ...
    D --> E{③.5 该形状 8×L×W 的图已捕获?<br>available_graph_buckets}
    E -->|未捕获| F[立即 warmup_bucket 请求预捕获<br>捕获在后台进行，藏在攒批等待里]
    E -->|已捕获| G{④ 引擎空闲或 on_tick<br>触发组批检查}
    F --> G
    G -->|4a 队列为空| N[什么都不发<br>等下一个请求]
    G -->|4a 队列有请求| H{4b 发车时目标 bucket 的图已就绪?<br>8 或 4}
    H -->|未就绪| W[等捕获完成 或 该批走 eager 并记账]
    H -->|已就绪| I{4c 满足发车条件?<br>满批 ≥ min_batch / 高优 / deadline / 超时}
    I -->|不满足| J[继续等<br>下次再查]
    I -->|满足| K[4d 取队列头部 ≤ max_batch 个请求]
    K --> L[生成 BatchSpec<br>batch_id / request_ids / bucket / deadline / graph_keys]
    L --> M[submit_batch 交给引擎]
```

要点：**请求一到就先确保 8×L×W 的图开始捕获（后台并行），发车前再确认目标 bucket 图已就绪**——捕获藏在攒批等待里，这是插件与成图衔接的核心。
...
sequenceDiagram
    ...
    P->>E: warmup_bucket(8 × 1024)（未捕获时，到达即触发）
    E->>G: 后台开始捕获 8×1024 全套图（prefill + step1..3）
    Note over P: 攒批或超时（捕获与等待并行）
    P->>P: 组批：生成 BatchSpec<br>bucket=1024，graph_keys=prefill:1024 + step1..3:4
    P->>E: submit_batch(spec)
    E->>G: 确认 bucket 图已就绪 + 预分配 KV 池稳定切片
    E->>K: 执行 batch
...
flowchart TD
    ...
    J -->|是| L{4 的图已就绪?}
    L -->|是| M[发 4：降级发车]
    L -->|否| N[等捕获完成 或 走 eager（兜底）]
```

说明：这里的“图已就绪”依赖 2.3 的**请求到达即捕获**（8×L×W 在首个请求到达时就已开始后台捕获）；正常情况下发车前 8 已捕获完成，4 也已由降级触发或空闲预热完成。本图中的“等捕获/走 eager”只是兜底分支。
...
sequenceDiagram
    ...
    P->>E: warmup_bucket（该形状未捕获时，到达即触发）
    E->>E: 后台捕获（与攒批并行）
...
> 两家策略逻辑的大白话总结已放到第 0 节；这里只保留源码位置便于追溯。

- **NVIDIA**：
  - [continuous.py](/tmp/nvidia_continuous.py:547)：`_plan_decode_batches`（阶段分组）、[L2292](/tmp/nvidia_continuous.py:2292)：`_forward_decode_step`
  - [decode_cuda_graph.py](/tmp/nvidia_decode_cuda_graph.py:401)：`_key`（图 key）、[L50](/tmp/nvidia_decode_cuda_graph.py:50)：LRU 上限、[L192](/tmp/nvidia_decode_cuda_graph.py:192)：命中提升、[L227](/tmp/nvidia_decode_cuda_graph.py:227)：存储/淘汰
  - [prefill_cuda_graph.py](/tmp/nvidia_prefill_cuda_graph.py:51)：LRU 上限、[L182](/tmp/nvidia_prefill_cuda_graph.py:182)：piecewise 分段
- **vllm-ACS**：
  - [hstu.py](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:577)：`forward_aclgraph`（运行时选档/replay）、[L864](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:864)：`_capture_model`（分图捕获）、[L1038](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:1038)：`capture_model`（全形状预捕获）、[L1050](/tmp/vllm-ACS/vllm-ascend/vllm_ascend/models/hstu/hstu.py:1050)：bs 档
  - [kv_cache_manager.py](/tmp/vllm-ACS/vllm/vllm/v1/core/kv_cache_manager.py:403)：`KVCacheManagerForGR`（uid 用户 KV）
  - [llm.py](/tmp/vllm-ACS/vllm/vllm/entrypoints/llm.py:907)：`beam_search_parallel`（调用方组 step）
  - [ag_demo.py](/tmp/vllm-ACS/model_zoo/gr-ag/ag_demo.py:118)：`generate_seq_stages`（长度档生成）
- **主文档**：[scheduler_strategy_design.md](/home/public/recif_beam_search/vllm-gr/docs/scheduler_strategy_design.md:23)（三库调度策略分析、1.5 成图对比、1.6 集中成图可行性）
